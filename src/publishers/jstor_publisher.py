...
Implementation for downloading papers from JSTOR with institutional access.
JSTOR hosts academic journals, books, and primary sources.
"""
...
class JSTORPublisher:
    """JSTOR publisher with browser automation for institutional access"""
    ...
    def can_handle_doi(self, doi: str) -> bool:
        ...
        return any(prefix in doi.lower() for prefix in [
            '10.2307/',  # Main JSTOR DOI prefix
            '10.1525/',  # University of California Press
            '10.1353/',  # Project MUSE (sometimes on JSTOR)
        ])
    ...
    async def download_paper(self, paper_identifier: str, save_path: Path) -> DownloadResult:
        ...
        try:
            ...
            if paper_identifier.startswith('http'):
                ...
                # Assume it's a JSTOR stable URL
                target_url = f"https://www.jstor.org/stable/{paper_identifier}"
            
            logger.info(f"JSTOR: Starting download for {target_url}")
            ...
            logger.error(f"JSTOR: Download error - {e}")
            return DownloadResult(
                success=False,
                error_message=f"JSTOR download failed: {str(e)}"
            )
    ...
    async def _browser_institutional_download(self, target_url: str, save_path: Path) -> DownloadResult:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(f"JSTOR: Navigating to {target_url}")
                ...
                if any(indicator in page_content.lower() for indicator in [
                    ...
                    'register & read', 'read online'
                ]):
                    logger.info("JSTOR: Institutional access required")
                    ...
                    if institutional_link:
                        logger.info("JSTOR: Clicking institutional access")
                        ...
                        if await self._handle_institutional_authentication(page):
                            logger.info("JSTOR: Institutional authentication successful")
                        else:
                            logger.warning("JSTOR: Institutional authentication may have failed")
                    else:
                        logger.warning("JSTOR: No institutional access link found")
                ...
                if pdf_downloaded:
                    ...
                    if save_path.exists() and save_path.stat().st_size > 1000:
                        ...
                        with open(save_path, 'rb') as f:
                            if f.read(4) == b'%PDF':
                                logger.info(f"JSTOR: PDF successfully downloaded to {save_path}")
                                return DownloadResult(
                                    success=True,
                                    file_path=save_path
                                )
                ...
                return DownloadResult(
                    success=False,
                    error_message="Could not download PDF from JSTOR"
                )
    ...
    async def _handle_institutional_authentication(self, page: Page) -> bool:
        ...
        try:
            ...
            logger.info(f"JSTOR auth: Current URL is {current_url}")
            
            # Check if we're on JSTOR's institutional selection page
            if 'jstor.org' in current_url and ('wayf' in current_url or 'institution' in current_url or 'shibboleth' in current_url):
                logger.info("JSTOR: On institutional selection page")
                ...
                for selector in eth_selectors:
                    try:
                        ...
                        if eth_element:
                            logger.info(f"JSTOR: Found ETH option with: {selector}")
                ...
                if not eth_found:
                    ...
                    for selector in search_selectors:
                        try:
                            ...
                            if search_box:
                                logger.info("JSTOR: Found institution search box")
            ...
            logger.warning(f"JSTOR: Institutional authentication error - {e}")
            return False
    ...
    async def _handle_eth_login(self, page: Page) -> bool:
        ...
        try:
            logger.info("JSTOR: Handling ETH login")
            ...
            if username_field and password_field:
                logger.info("JSTOR: Found ETH login fields")
            ...
            logger.warning(f"JSTOR: ETH login error - {e}")
            return False
    ...
    async def _handle_shibboleth_auth(self, page: Page) -> bool:
        ...
        try:
            logger.info("JSTOR: Handling Shibboleth authentication")
            return await self._handle_eth_login(page)
            
        except Exception as e:
            logger.warning(f"JSTOR: Shibboleth auth error - {e}")
            return False
    
    async def _find_and_download_pdf(self, page: Page, save_path: Path) -> bool:
        """Find and download PDF from JSTOR article page"""
        
        try:
            # JSTOR PDF download patterns
            pdf_selectors = [
                'a[href$=".pdf"]',
                'a[href*="/pdf/"]',
                'a:has-text("PDF")',
            ...
            if not pdf_link:
                logger.warning("JSTOR: No PDF download link found")
            ...
            async def handle_download(download):
                ...
                try:
                    ...
                    logger.info(f"JSTOR: PDF saved to {save_path}")
                except Exception as e:
                    logger.error(f"JSTOR: Download save failed - {e}")
            ...
            logger.error(f"JSTOR: PDF download failed - {e}")
            return False
        ...
        try:
            ...
            async def handle_download(download):
                ...
                try:
                    await download.save_as(save_path)
                    download_completed = True
                    logger.info(f"JSTOR: PDF saved from URL to {save_path}")
                except Exception as e:
                    logger.error(f"JSTOR: URL download save failed - {e}")
            ...
            logger.error(f"JSTOR: PDF URL download failed - {e}")
            return False
    
    async def search_papers(self, query: str, max_results: int = 10) -> List[Dict]:
        """Search for papers on JSTOR (optional functionality)"""
        ...
        try:
            async with async_playwright() as p:
                ...
                # JSTOR search URL
                search_url = f"https://www.jstor.org/action/doBasicSearch?Query={query}"
                await page.goto(search_url, wait_until='domcontentloaded')
                
                # Parse search results
                article_elements = await page.query_selector_all('.result-item')
                    ...
                    try:
                        ...
                        if title_elem:
                            ...
                            if link and not link.startswith('http'):
                                link = urljoin('https://www.jstor.org/', link)
                        ...
                        logger.debug(f"JSTOR: Error parsing search result - {e}")
                        continue
                
                await browser.close()
                
        except Exception as e:
            logger.error(f"JSTOR: Search failed - {e}")
        
        return results

    def __str__(self):
        return f"JSTORPublisher(auth={self.auth_config.institutional_login})"