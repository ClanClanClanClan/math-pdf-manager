...
PATHOLOGIES: list = [
    ...
    # Found by the exhaustive multi-agent sweep of the case space, all
    # reproduced by hand before being fixed.
    ("Ito_K_1944_On_stochastic_differential_equations.pdf",
     "snake_case year: '_1' must not subscript when a digit follows"),
    ("A, B. - L^23 estimates.pdf",
     "two-digit exponent: 'L²3' is half-raised"),
    ("A, B. - Series Sigma 2^{-n_k} converges.pdf",
     "nested inside a digit-based carrier"),
    ("A, B. - Norm |X|_{L^2} bounded.pdf",
     "nested inside a symbol-based carrier"),
    ("A, B. - X_{Doob - Meyer} decomposition.pdf",
     "the author/title separator INSIDE a script must never be welded"),
    ("A, B. - The W^{1}- Sobolev embedding.pdf",
     "conversion must not create a pattern a later rule rewrites"),
    ("A, B. - e^{-x} decay of solutions.pdf",
     "a symbol is not a sentence start: 'E⁻ˣ' is a different quantity"),
    ("A, B. - e^x growth estimates.pdf", "same, with a modifier-letter script"),
