...
from scripts.vpn.bulletproof_vpn_connect import BulletproofVPNConnect
...
class CompleteAutoPDFSystem:
    ...
    async def download_pdf_automatically(self, doi, journal):
        ...
        async with async_playwright() as p:
            browser = await p.chromium.launch(
                headless=False, downloads_path=str(self.downloads_dir)  # Show browser for debugging
            )

            context = await browser.new_context(
                user_agent="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)", accept_downloads=True
            )
    ...
    async def run_complete_automation(self):
        ...
        for i, paper in enumerate(papers, 1):
            ...
            result = await self.download_pdf_automatically(paper["doi"], paper["journal"])
