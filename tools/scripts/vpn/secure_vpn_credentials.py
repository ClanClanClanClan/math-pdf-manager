...
if __name__ == "__main__":
    ...
    env_pwd = os.getenv("VPN_PASSWORD", "").strip()
    if not env_pwd:
        try:
            env_pwd = getpass.getpass("Enter VPN password (will be stored securely): ")
        except Exception:
            env_pwd = ""
    if not env_pwd:
        print("❌ No password provided. Set VPN_PASSWORD or enter interactively.")
        raise SystemExit(1)
    store_vpn_password(env_pwd)
