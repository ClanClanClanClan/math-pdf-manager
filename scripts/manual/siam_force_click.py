...
Fix the click interception issue with force click.
"""
...
async def siam_force_click():
    print("🎯 SIAM WITH FORCE CLICK FIX")
    print("=" * 40)
    ...
    try:
        async with async_playwright() as p:
            ...
            print("\n🔄 STEP 3: Force click 'Access via your Institution'")
            ...
            try:
                ...
                print("   Force click failed, trying JavaScript click...")
                # Fallback to JavaScript click
                await page.evaluate('document.querySelector(\'a:has-text("Access via your Institution")\').click()')
            ...
            async def handle_download(download):
                ...
                filename = f"siam_{test_doi.replace('/', '_').replace('.', '_')}_FORCE_CLICK.pdf"
...
async def main():
    success = await siam_force_click()
    
    if success:
        print("\n🎉 SIAM WORKING WITH FORCE CLICK!")
    else:
        print("\n❌ Still having issues")
